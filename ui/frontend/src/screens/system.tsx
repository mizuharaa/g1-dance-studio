...
function InstanceStrip({ instances }: { instances: NonNullable<ConsoleData["system"]>["instances"] }) {
  if (!instances || instances.length < 2) return null
  return <Card>
    <CardHeader className="pb-3"><div className="panel-kicker"><Cpu /> Instances</div><CardTitle className="mt-2">Parallel training boxes ({instances.length})</CardTitle></CardHeader>
    <CardContent><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {instances.map((inst, i) => {
        ...
        return <div key={inst.name ?? i} className="rounded-lg border border-border bg-background/25 p-3.5">
          <div className="flex items-center justify-between gap-2">
            <span className="truncate text-xs font-semibold">{inst.name ?? `box ${i + 1}`}</span>
            <span className={`h-2 w-2 shrink-0 rounded-full ${online ? "bg-emerald-400" : inst.stale ? "bg-amber-400" : "bg-red-400"}`} title={inst.detail ?? ""} />
          </div>
          <div className="mt-1 font-mono text-[10px] text-muted-foreground">GPU {inst.gpu?.utilization_pct != null ? `${Math.round(inst.gpu.utilization_pct)}%` : "offline"}{inst.stale ? " · stale" : ""}</div>
          {job ? <><Progress className="mt-2 h-1.5" value={pct} indicatorClassName={job.running ? undefined : "bg-emerald-500"} />
            <div className="mt-1.5 flex justify-between font-mono text-[10px] text-muted-foreground"><span className="truncate">{job.name}</span><span className="shrink-0">{job.iteration?.toLocaleString() ?? "—"}/{job.max_iteration?.toLocaleString() ?? "—"}</span></div></>
            : <div className="mt-2 text-[10px] text-muted-foreground">{online ? "no active trainer (provisioning / idle)" : "unreachable"}</div>}
        </div>
      })}
    </div></CardContent>
  </Card>
...
function SystemOverview({ data }: { data: ConsoleData }) {
  ...
  return <div className="space-y-4">
    {!data.system?.reachable && <InlineAlert tone="warning" title="Primary box unreachable" body={data.system?.detail ?? "Training may still be running; this panel is showing the last known state."} />}
    <InstanceStrip instances={data.system?.instances} />
