...
# capability limit). See cloud/sim2real_task_v13.py header.
#
...
#
# Push first (laptop) — NOTE the v12_bundle dir must be scp'd too:
#   scp -P <PORT> -i .secrets/greennode_rsa cloud/{sim2real_task_v13.py,\
#     train_v13_curriculum.sh,run_attempt10.sh} root@<IP>:/workspace/notebook-data/cloud/
...
#   cd /workspace/notebook-data && nohup bash cloud/run_attempt10.sh > attempt10.out 2>&1 &
#
...
G1_SLOWDOWN=1.6667 G1_OBS_HISTORY=5 G1_PHASE_OBS=${G1_PHASE_OBS:-0} \
  "$PY" "$NB/cloud/sim2real_task_v13.py" --selfcheck || die "selfcheck FAIL"
...
G1_CMD_DELAY_MAX_LAG=4 G1_OBS_DELAY_MAX_LAG=1 G1_DRIFT_TERM_M=0.8 \
  "$PY" "$NB/cloud/sim2real_task_v13.py" Mjlab-Tracking-Flat-Unitree-G1-S2R-V13 \
    --env.scene.num-envs 64 --env.commands.motion.motion-file "$NB/motions/thriller_v12_060.npz" \
    --agent.max-iterations 3 --agent.run-name smoketest-v13 || die "smoke test FAIL"
...
G1_SLOWDOWN=1.6667 G1_OBS_HISTORY=5 G1_PHASE_OBS=${G1_PHASE_OBS:-0} \
  "$PY" "$NB/cloud/verify_obs_layout.py" --task Mjlab-Tracking-Flat-Unitree-G1-S2R-V13 \
    --task-module sim2real_task_v13 --motion-file "$NB/motions/thriller_v12_100.npz" \
    --num-envs 2
_OBS_RC=$?
...
export RUN_NAME=${RUN_NAME:-train-thriller_v13style-$(date +%m%d)}
...
M100=$NB/motions/thriller_v12_100.npz \
  bash "$NB/cloud/train_v13_curriculum.sh"
say "run_attempt10 DONE — pull exports, judge vs calibrated bars, sign, DELETE THE BOX"
