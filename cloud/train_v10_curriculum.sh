...
ENTRY=$NB/cloud/sim2real_task_v10.py
TASK=Mjlab-Tracking-Flat-Unitree-G1-S2R-V10
TM=sim2real_task_v10
...
RUN=${RUN_NAME:-train-thriller_v10spd-$(date +%m%d)}
...
echo "===== STAGE 3/4  tempo 0.90x, 0-50 ms, drift<0.5 m, +2250 (resume s2)  $(date -Is) ====="
...
G1_SLOWDOWN=1.1111 G1_CMD_DELAY_MAX_LAG=10 G1_OBS_DELAY_MAX_LAG=2 G1_DRIFT_TERM_M=0.5 \
...
echo "===== STAGE 4/4  tempo 1.00x NATIVE, 0-60 ms, drift<0.4 m, +2250 (resume s3)  $(date -Is) ====="
...
G1_SLOWDOWN=1.0 G1_CMD_DELAY_MAX_LAG=12 G1_OBS_DELAY_MAX_LAG=3 G1_DRIFT_TERM_M=0.4 \
...
echo "  survival >=99%, drift <=1.0 m, 40ms+push survival >=95%. Also judge vs the"
echo "  calibrated anchor row (experiments/REGISTRY.md): anchor == ~70% IRL quality."
echo "  Beat v8 (99.2% nominal / 59.4% @40ms+push / drift 4.31) at NATIVE tempo."
