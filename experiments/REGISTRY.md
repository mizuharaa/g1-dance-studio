...
| `thriller_v9adpt` (iter 9500) | 2026-07-17 | thriller_grounded_adaptive.npz (3767f, 75.3s, 49% native speed) | v8 recipe verbatim + G1_WAIST_WINDOWS (adaptive warp) | model_9500.pt (best of 6; iter-11000 had collapsed to 45% — picker saved it again) | `exports/train-thriller_v9adpt-0717/gap.json` | nominal surv 89.8%, rr_mpkpe 0.064, drift 3.26, ankle p95 20.96, 40ms+push 43.0%; heldout 96.5/82.4 | **still above the deployed-quality bar** (anchor 34.4% @40ms+push) but **WORSE than v8** on survival/push | KEY FINDING: ankle p95 ~21 Nm UNCHANGED from v8 despite motion demand dropping 67.8→34 Nm — the residual ankle torque is the policy's stabilization effort, NOT choreography demand; slowing the motion further won't cut it. v9 trades robustness for speed (dances 49% at native tempo, 75s vs 89s). |
| `thriller_v8s2r` (iter 10000) | 2026-07-17 | thriller_grounded_repaired_1p8x.npz (4435f, sha csv `48bcc1e7…`) | `cloud/sim2real_task_v8.py` (770-dim actor) | model_10000.pt (screened 6, picker worked) | `exports/train-thriller_v8s2r-0716/gap.json` | nominal surv **99.2% (first-ever PASS)**, rr_mpkpe **0.059 (best)**, drift 4.31 (FAIL), ankle p95 20.4 (FAIL), 40ms+push 59.4% (FAIL vs bar) | **vs calibrated anchor: 40ms+push 59.4% vs anchor's 34.4% = 1.7× more robust than the proven-deployable policy**; drift honest (actor can't see position — v7's 0.81 used a privileged term unavailable IRL) | Gate FAIL on absolute bars, but bars are now known-miscalibrated (see anchor row). Winner exported via export_ckpt_onnx (matches gate). Deploy blocked on robot repair + 770-dim deploy wave. |
| `thriller_csv_ankle_penalty` **CALIBRATION RE-RUN** | 2026-07-17 | anchor_motion.npz (from its deploy csv) | (its own 2026-07-08 recipe, via `--onnx` gate mode) | — (onnx sha `444864f9…`) | `exports/train-thriller_v8s2r-0716/calibration_anchor_gap.json` | nominal surv 100%, mpkpe 0.154, drift 0.40, ankle p95 10.56 — **reproduces the 07-08 gate numbers exactly → gate NOT hallucinated**; delay40ms 37.5%, 40ms+push **34.4%** | **THE MAPPING: gate-perfect nominal + 34% @40ms+push ↔ ~70% mimicry IRL (live-deployed)** | Gate latency bars (≥95% @40ms+push) are far stricter than deployed reality — judge policies RELATIVE to this anchor, not the absolute bars. |
...
| `thriller_g1_repaired.csv` (`0d3ffc28492b5e50`) | thriller_g1_clean | **global slowdown 2.5×** (pure; music stays synced under uniform time-stretch) | **39.4** [verified] | 22.3 | **0.0%** [verified] | **0.999** | `experiments/motion_feasibility/thriller_g1_repaired_scorecard.json` |
| `thriller_g1_grounded_adaptive.csv` (2026-07-17, **v9 motion**) | thriller_g1_grounded | **adaptive time-warp** (demand-shaped local retiming, `motion_repair.py --adaptive`): only over-torque moments slowed, **49% of the dance stays native 1.0×**, overall 1.53× (vs 1.8× uniform), max local gain 2.17× | **34.0** [verified] | 32.3 | **0.0%** [verified] | poses identical (pure time-warp) | `data/motions/thriller/thriller_g1_grounded_adaptive_scorecard.json` (carries `time_map` → warped waist windows 16.72–24.81, 35.12–53.5) |
